...
# Configuration
CRAWLER_DATA_DIR = Path(__file__).parent.parent / "RawHTMLdata"  # A_Search_Engine/RawHTMLdata
...
def _file_processor_worker(file_path: Path) -> Dict[str, List[Dict[str, Any]]]:
    """Optimized worker function using persistent processor instance."""
    global _worker_processor
    ...
    try:
        ...
        return {
            ...
            "chunks": all_chunks,
            "stats": _worker_processor.get_processing_stats()
        }
...
class DataPipeline:
    ...
    def initialize(self) -> bool:
        ...
        # Create OpenSearch daily indices if needed
        if not self.indexer.create_indices_if_needed():
    ...
    def process_files_batch(self, new_files: List[Path]) -> bool:
        ...
        try:
            # Process files in parallel with optimized worker initialization
